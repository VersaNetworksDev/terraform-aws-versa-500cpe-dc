...
post-up route add -net ${DR_mgmt_subnet} gw ${dc_router_mgnt_ip}
...
route add -net ${DR_mgmt_subnet} gw ${dc_router_mgnt_ip}
dcuuid=$(uuidgen)
druuid=$(uuidgen)
...
then
    ...
    touch \$ha_log_path
echo -e "HA sleep started" >> \$ha_log_path
sleep 50
echo -e "HA sleep ended" >> \$ha_log_path    
...
sleep 20
echo "DC-ROUTER local resource creation Started ..." >> \$ha_log_path
curl -v -k -u Administrator:versa123 --header "Content-Type: application/json" https://${master_dir_mgmt_ip}:9182/nextgen/localorganization -d '{"localOrg":{"name":"DC-ROUTER","uuid":"$dcuuid","localOrgNetworks":[{"name":"Master_director","ipAddressAllocationMode":"manual","ipAddressAllocationModev6":"manual"}],"localInstances":[{"name":"DC-ROUTER","ipAddress":"${DC_router_eth0_mgnt_ip}"}]}}' >> /dev/null 2>&1 >> \$ha_log_path
echo "DC-ROUTER local resource creation completed .." >> \$ha_log_path
echo "DR-ROUTER local resource creation Started ..." >> \$ha_log_path
curl -v -k -u Administrator:versa123 --header "Content-Type: application/json" https://${master_dir_mgmt_ip}:9182/nextgen/localorganization -d '{"localOrg":{"name":"DR-ROUTER","uuid":"$druuid","localOrgNetworks":[{"name":"Master_director","ipAddressAllocationMode":"manual","ipAddressAllocationModev6":"manual"}],"localInstances":[{"name":"DR-ROUTER","ipAddress":"${DR_router_eth0_mgnt_ip}"}]}}' >> /dev/null 2>&1 >> \$ha_log_path
echo "DR-ROUTER local resource creation completed .." >> \$ha_log_path
echo "Org Configuration started .." >> \$ha_log_path
curl -v -k -u Administrator:versa123 --header "Content-Type: application/json" https://${master_dir_mgmt_ip}:9182/nextgen/organization -d '{"id":1,"name":"${parent_org_name}","authType":"psk","dynamicTenantConfig":{"inactivityInterval":48},"subscriptionPlan":"Default-All-Services-Plan","vrfsGroups":[{"name":"${parent_org_name}-LAN-VR","vrfId":"1","enable_vpn":"true"}],"analyticsClusters":["VAN-Cluster"],"sharedControlPlane":false,"cpeDeploymentType":"SDWAN","cmsOrgs":[{"name":"DC-ROUTER","uuid":"$dcuuid"},{"name":"DR-ROUTER","uuid":"$druuid"}]}}' >> /dev/null 2>&1 >> \$ha_log_path
echo "Org Configuration completed .." >> \$ha_log_path
sleep 20
orguuid=\$(curl -k -u Administrator:versa123 https://${master_dir_mgmt_ip}:9182/vnms/organization/orgs --silent | json_pp | grep uuid | awk  '{print \$3}' | tr -d '"' | tr -d ",")
cat >> dc_router_json << EOT
{"add-devices":{"devices":{"device":{"mgmt-ip":"${DC_router_eth0_mgnt_ip}","name":"DC-ROUTER","org":"\$orguuid","cmsorg":"$dcuuid","type":"service-vnf","discover":"true","snglist":{"sng":[{"name":"Default_All_Services","isPartOfVCSN":true}]},"subscription":{"solution-tier":"NGFW","bandwidth":50,"license-period":1,"custom-param":[]}}}}}
EOT
echo "DC-ROUTER device addition started .." >> \$ha_log_path
curl -v -k -u Administrator:versa123 --header "Content-Type: application/json" https://${master_dir_mgmt_ip}:9182/api/config/nms/actions/_operations/add-devices -d "@dc_router_json" >> /dev/null 2>&1 >> \$ha_log_path
echo "DC-ROUTER device addition complted .." >> \$ha_log_path
sleep 20
cat >> dr_router_json << EOT
{"add-devices":{"devices":{"device":{"mgmt-ip":"${DR_router_eth0_mgnt_ip}","name":"DR-ROUTER","org":"\$orguuid","cmsorg":"$druuid","type":"service-vnf","discover":"true","snglist":{"sng":[{"name":"Default_All_Services","isPartOfVCSN":true}]},"subscription":{"solution-tier":"NGFW","bandwidth":50,"license-period":1,"custom-param":[]}}}}}
EOT
echo "DR-ROUTER device addition started .." >> \$ha_log_path
curl -v -k -u Administrator:versa123 --header "Content-Type: application/json" https://${master_dir_mgmt_ip}:9182/api/config/nms/actions/_operations/add-devices -d "@dr_router_json" >> /dev/null 2>&1 >> \$ha_log_path
echo "DR-ROUTER device addition complted .." >> \$ha_log_path
echo "Adding static routes for overlay subnets ..." >> \$ha_log_path
curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X POST https://${master_dir_mgmt_ip}:9182/api/config/nms/routing-options/static -d '{"route":{"description":"overlay_prifix_dc","destination-prefix":"${overlay_prefixes}","next-hop-address":"${dc_router_south_bound_ip}","outgoing-interface":"eth1"}}' >> /dev/null 2>&1 >> \$ha_log_path
curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X POST https://${master_dir_mgmt_ip}:9182/api/config/nms/routing-options/static -d '{"route":{"description":"overlay_prifix_dr","destination-prefix":"${overlay_prefixes}","next-hop-address":"${dr_router_south_bound_ip}","outgoing-interface":"eth1"}}' >> /dev/null 2>&1 >> \$ha_log_path
...
curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X DELETE https://${master_dir_mgmt_ip}:9182/vnms/ipam/overlay/prefixes/1
echo "Overlay Address Prefixes creation ..." >> /dev/null 2>&1 >> \$ha_log_path
curl -k -u Administrator:versa123 --header "Content-Type: application/json" https://${master_dir_mgmt_ip}:9182/vnms/ipam/overlay/prefixes -d '{"prefix": "${overlay_prefixes}","status": {"label": "Active"}}' >> /dev/null 2>&1 >> \$ha_log_path
cat >> controller_1_json << EOT
{"versanms.sdwan-controller-workflow": {"controllerName": "${controller_1_hostname}","orgName": "${parent_org_name}","siteId": "1","stagingController": "true","postStagingController": "true","resourceType": "Baremetal","analyticsCluster": "VAN-Cluster","ipv4dhcp": "false","locationInfo": {"country": "${controller_1_country}"},"bgp":{"peerIP":"${dc_router_south_bound_ip}","peerIPAs":"64002"},"baremetalController": {"serverIP": "${controller_1_mgnt_ip}","controllerInterface": {"interfaceName": "vni-0/0","unitInfoList": [{"networkName": "To_Director","vlanId": 0,"ipv4address": ["${controller_1_south_bound_ip}$controller_1_south_bound_network"],"ipv6gateway": "","ipv4gateway": "${master_dir_south_bound_ip}"}]},"wanInterfaces": [{"interfaceName": "vni-0/1","unitInfoList": [{"ipv4address": ["${controller_1_internet_private_ip}$controller_1_internet_network"],"vlanId": "0","networkName": "Internet","ipv4dhcp": false,"ipv6dhcp": false,"ipv4gateway": "${DC_internet_subnet_gateway}","publicIPAddress": "${controller_1_internet_public_ip}","wanStaging": true,"poolSize": "128","transportDomainList": ["Internet"]}]}]}}} >> /dev/null 2>&1 >> \$ha_log_path
EOT
orguuid=\$(curl -k -u Administrator:versa123 https://${master_dir_mgmt_ip}:9182/vnms/organization/orgs --silent | json_pp | grep uuid | awk  '{print \$3}' | tr -d '"' | tr -d ",")
echo -e "\n creating Wan Networks ..." >> \$ha_log_path
echo \`curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X POST https://${master_dir_mgmt_ip}:9182/nextgen/organization/\$orguuid/wan-networks -d '{"name":"Internet","transport-domains":["Internet"],"description":""}'\` >> /dev/null 2>&1 >> \$ha_log_path
echo "controller_1 save initiated ..." >> \$ha_log_path
curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X POST https://${master_dir_mgmt_ip}:9182/vnms/sdwan/workflow/controllers/controller -d "@controller_1_json" >> /dev/null 2>&1 >> \$ha_log_path
sleep 10
echo "Deploying controller 1 ..." >> \$ha_log_path
curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X POST https://${master_dir_mgmt_ip}:9182/vnms/sdwan/workflow/controllers/controller/deploy/${controller_1_hostname} >> /dev/null 2>&1 >> \$ha_log_path
echo -e "\ncontroller 1 deployment completed ..." >> \$ha_log_path
sleep 10
echo "controller 2 save initiated ..." >> \$ha_log_path
curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X POST https://${master_dir_mgmt_ip}:9182/vnms/sdwan/workflow/controllers/controller -d '{"versanms.sdwan-controller-workflow": {"controllerName": "${controller_2_hostname}","orgName": "${parent_org_name}","siteId": "2","peerControllers":["${controller_1_hostname}"],"stagingController": "true","postStagingController": "true","resourceType": "Baremetal","analyticsCluster": "VAN-Cluster","ipv4dhcp": "false","locationInfo": {"country": "${controller_2_country}"},"bgp":{"peerIP":"${dr_router_south_bound_ip}","peerIPAs":"64002"},"baremetalController": {"serverIP": "${controller_2_mgnt_ip}","controllerInterface": {"interfaceName": "vni-0/0","unitInfoList": [{"networkName": "To_Director","vlanId": 0,"ipv4address": ["${controller_2_south_bound_ip}$controller_2_south_bound_network"],"ipv6gateway": "","ipv4gateway": "${slave_dir_south_bound_ip}"}]},"wanInterfaces": [{"interfaceName": "vni-0/1","unitInfoList": [{"ipv4address": ["${controller_2_internet_private_ip}$controller_2_internet_network"],"vlanId": "0","networkName": "Internet","ipv4dhcp": false,"ipv6dhcp": false,"ipv4gateway": "${DR_internet_subnet_gateway}","publicIPAddress": "${controller_2_internet_public_ip}","wanStaging": true,"poolSize": "128","transportDomainList": ["Internet"]}]}]}}}' >> /dev/null 2>&1 >> \$ha_log_path
sleep 10
echo "Deploying controller 2 ..." >> \$ha_log_path
curl -k -u Administrator:versa123 --header "Content-Type: application/json" -X POST https://${master_dir_mgmt_ip}:9182/vnms/sdwan/workflow/controllers/controller/deploy/${controller_2_hostname} >> /dev/null 2>&1 >> \$ha_log_path
